...
const SheltersMap = ({ shelters }) => {
  ...
  const organizations = useMemo(
    () =>
      Array.from(
        new Set(shelters.map((s) => s.organization_name).filter(Boolean)),
      ),
    [shelters],
  );
  ...
  const filteredShelters = useMemo(() => {
    return filterSheltersWithOccupancy({
      ...
      if (filters.shelterType && s.shelter_type !== filters.shelterType)
        return false;
      if (filters.organization && s.organization_name !== filters.organization)
        return false;
      return true;
    });
    ...
    <div className="shelters-map-container">
      ...
      <div className="shelters-map-count">
        <span className="highlight-program-number">
          {filteredShelters.reduce((acc, s) => acc + s.programs.length, 0)}
        </span>{" "}
        programs across{" "}
